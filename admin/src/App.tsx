    ...
    <>
      ...
      <div className="grid2">
        <div className="panel"><h3>Údaje</h3><div style={{ padding: 16 }}>
          ...
          {data.property?.inventoryUnit === "bed" ? <div className="kvline"><span className="muted">Energie (vzdušné){!r.energyFeeExempt && Number(data.property.energyFeePerNight) > 0 ? ` · ${money(Number(data.property.energyFeePerNight) * r.nights)}` : ""}</span><label className="row" style={{ gap: 6 }}><input type="checkbox" checked={!!r.energyFeeExempt} disabled={busy} onChange={(e) => run(() => api.setReservationEnergyExempt(id, e.target.checked))} /> osvobozeno (neúčtovat)</label></div> : null}
