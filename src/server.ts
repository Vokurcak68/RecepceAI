...
adminRouter.get("/reception", h((_req, res) => admin.receptionToday(pid(res))));
...
adminRouter.post("/room-types", h((req, res) => { const b = z.object({ name: z.string().min(1), description: z.string().optional(), capacityAdults: z.number().int().positive(), capacityChildren: z.number().int().nonnegative().optional(), maxExtraBeds: z.number().int().min(0).max(10).optional(), extraBedPrice: z.number().nonnegative().optional(), basePrice: z.number().nonnegative(), weeklyPrice: z.number().nonnegative().optional(), monthlyPrice: z.number().nonnegative().optional(), amenities: z.array(z.string()).optional() }).parse(req.body); return admin.createRoomType(pid(res), b); }));
adminRouter.patch("/room-types/:id", h((req) => { const b = z.object({ name: z.string().optional(), description: z.string().optional(), capacityAdults: z.number().int().optional(), capacityChildren: z.number().int().optional(), maxExtraBeds: z.number().int().min(0).max(10).optional(), extraBedPrice: z.number().nonnegative().optional(), basePrice: z.number().optional(), weeklyPrice: z.number().optional(), monthlyPrice: z.number().optional(), amenities: z.array(z.string()).optional() }).parse(req.body); return admin.updateRoomType(req.params.id, b); }));
...
adminRouter.post("/documents/bulk-invoice", h((req, res) => {
  const b = z.object({ reservationIds: z.array(z.string().uuid()).min(1), companyId: z.string().uuid().optional(), preview: z.boolean().optional() }).parse(req.body);
  return billing.issueBulkInvoice(pid(res), b.reservationIds, b.companyId, { preview: b.preview });
}));
...
adminRouter.post("/reservations/:id/proforma", h(async (req, res) => {
  ...
  const doc = await billing.issueProforma(pid(res), req.params.id, b.amount, b.dueInDays, { preview: b.preview });
  if (b.preview) return doc; // náhled — bez zápisu a bez e-mailu
