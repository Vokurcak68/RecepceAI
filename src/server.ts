...
adminRouter.post("/room-types", h((req, res) => { const b = z.object({ name: z.string().min(1), description: z.string().optional(), capacityAdults: z.number().int().positive(), capacityChildren: z.number().int().nonnegative().optional(), maxExtraBeds: z.number().int().min(0).max(10).optional(), basePrice: z.number().nonnegative(), weeklyPrice: z.number().nonnegative().optional(), monthlyPrice: z.number().nonnegative().optional(), amenities: z.array(z.string()).optional() }).parse(req.body); return admin.createRoomType(pid(res), b); }));
adminRouter.patch("/room-types/:id", h((req) => { const b = z.object({ name: z.string().optional(), description: z.string().optional(), capacityAdults: z.number().int().optional(), capacityChildren: z.number().int().optional(), maxExtraBeds: z.number().int().min(0).max(10).optional(), basePrice: z.number().optional(), weeklyPrice: z.number().optional(), monthlyPrice: z.number().optional(), amenities: z.array(z.string()).optional() }).parse(req.body); return admin.updateRoomType(req.params.id, b); }));
