...
export const api = {
  notifyStaff: (joinUrl: string, propertyName: string) =>
    req<{ sent: boolean; callId?: string }>(`/call/notify`, { method: "POST", body: JSON.stringify({ joinUrl, propertyName }) }),
  resolveCall: (callId: string) =>
    req<{ resolved: boolean }>(`/call/resolve`, { method: "POST", body: JSON.stringify({ callId }) }),
  ...
  // ── D-ID real-time avatar (cesta C) ──
  didConfig: () => req<{ enabled: boolean; sourceUrl: string }>(`/did/config`),
  didCreateStream: () => req<DidStream>(`/did/streams`, { method: "POST" }),
  didSdp: (id: string, answer: unknown, session_id: string) =>
    req(`/did/streams/${id}/sdp`, { method: "POST", body: JSON.stringify({ answer, session_id }) }),
  didIce: (id: string, candidate: Record<string, unknown>, session_id: string) =>
    req(`/did/streams/${id}/ice`, { method: "POST", body: JSON.stringify({ candidate, session_id }) }),
  didTalk: (id: string, session_id: string, text: string, lang: string) =>
    req(`/did/streams/${id}/talk`, { method: "POST", body: JSON.stringify({ session_id, text, lang }) }),
  didClose: (id: string, session_id: string) =>
    req(`/did/streams/${id}/close`, { method: "POST", body: JSON.stringify({ session_id }) }),
};
