...
// Tento testovací frontend (kiosk-avatar) defaultně používá D-ID „digital human" (cesta C).
const AVATAR_PREF = localStorage.getItem("avatarVariant") || "did";
const USE_DID = AVATAR_PREF === "did";
// Záložní 2D/3D vizuál, když D-ID není dostupné (bez klíče) nebo pro ostatní varianty.
const AVATAR: AvatarVariant = (USE_DID ? "receptionist" : AVATAR_PREF) as AvatarVariant;
// Statická fotka recepčního (idle obrazovka i poster, než naběhne video).
const DID_POSTER = "/reception-daniel-2.jpg"; // v2 = vyšší rozlišení
const GREET_DELAY = 2500; // ms — odklad pozdravu po probuzení, ať se avatar/klip stihne načíst

// Volitelný layout úvodní obrazovky: "classic" (výchozí, beze změny) | "hero" (Daniel vlevo,
// tlačítka vpravo) | "bold" (velký portrét Daniela bez kruhu + skleněný panel).
// Zapne se přes ?home=hero / ?home=bold (uloží se → na kiosku drží), zpět ?home=classic.
const _homeParam = new URLSearchParams(window.location.search).get("home");
if (_homeParam) localStorage.setItem("homeLayout", _homeParam);
const HOME_LAYOUT = localStorage.getItem("homeLayout") || "classic";
const HERO_HOME = HOME_LAYOUT === "hero" || HOME_LAYOUT === "bold";
const HERO_BOLD = HOME_LAYOUT === "bold";
const HERO_EPIC = HOME_LAYOUT === "epic"; // Daniel přes celou výšku + částice + animace
...
export function App() {
  ...
  // D-ID avatar (cesta C): zjisti, zda je backend nakonfigurovaný (klíč v .env).
  const [didEnabled, setDidEnabled] = useState(false);
  useEffect(() => { if (USE_DID) api.didConfig().then((c) => setDidEnabled(c.enabled)).catch(() => setDidEnabled(false)); }, []);
  const stageRef = useRef<AvatarStageHandle | null>(null);
  const [avatarSpeaking, setAvatarSpeaking] = useState(false); // mluví Daniel (klip/živě)?
  const [greetingArmed, setGreetingArmed] = useState(false);   // čeká odložený pozdrav (mic ani titulky zatím ne)
  const didActive = USE_DID && didEnabled;
  const didLive = didActive && screen !== "idle"; // avatar v DOM (mimo idle)

  // Manifest předrenderovaných klipů pevných vět (cesta B): `${lang}|${text}` → URL.
  const [clipMap, setClipMap] = useState<Record<string, string>>({});
  useEffect(() => {
    fetch("/clips/clips.json?v=6").then((r) => r.ok ? r.json() : {}).then((m: Record<string, { lang: string; text: string; file: string }>) => {
      const map: Record<string, string> = {};
      Object.values(m).forEach((e) => { map[`${e.lang}|${e.text}`] = `/clips/${e.file}`; });
      setClipMap(map);
    }).catch(() => {});
  }, []);
  const clipFor = (text: string) => clipMap[`${lang}|${text.trim()}`];

  // Jednotné vyslovení: pevná věta = klip (0 kreditů), dynamika = živý D-ID, jinak prohlížečové TTS.
  const pendingSay = useRef<string[]>([]); // věty řečené dřív, než se AvatarStage namountuje (pozdrav)
  const greetTimer = useRef<ReturnType<typeof setTimeout>>(); // odložený pozdrav po probuzení
  const say = (text: string) => {
    if (didActive) {
      if (stageRef.current) stageRef.current.say(text, clipFor(text));
      else pendingSay.current.push(text);
    } else speak(text);
  };
  ...
  useEffect(() => {
    if (didLive && stageRef.current && pendingSay.current.length) {
      const q = pendingSay.current; pendingSay.current = [];
      q.forEach((tx) => stageRef.current!.say(tx, clipFor(tx)));
    }
  }, [didLive, clipMap]); // eslint-disable-line

  // Hlasové ovládání úvodního rozcestníku (poslech + vyhodnocení záměru).
  const recRef = useRef<{ start: () => void; stop: () => void } | null>(null);
  const { listening, start: startListen, stop: stopListen, supported: recSupported, interim: recInterim } = useRecognition(lang, (text) => {
  ...
  useEffect(() => {
    // PUSH-TO-TALK: mikrofon se NIKDY nespouští sám — jen ťuknutím na „Mluvit".
    // Automaticky ho jen VYPÍNÁME: když avatar mluví nebo host odejde z úvodu.
    if (listening && (screen !== "home" || speaking || avatarSpeaking)) stopListen();
  }, [screen, speaking, avatarSpeaking, listening]); // eslint-disable-line

  // Změna jazyka na úvodu → pozdrav znovu v novém jazyce (potvrdí i hlas).
  useEffect(() => {
    if (screen === "home") { setLine(t("welcome")); say(t("welcome")); }
  ...
  function go(s: Screen, sayKey?: Parameters<typeof t>[0], extra?: string) {
    clearTimeout(greetTimer.current); setGreetingArmed(false); // zruš čekající pozdrav, pokud uživatel odešel jinam
    setError("");
    setScreen(s);
  ...
  // Pozdrav odložíme o GREET_DELAY, ať se avatar/klip stihne načíst (jinak by mluvil dřív, než otevře pusu).
  function wake() {
    resetAll(); setScreen("home"); const w = t("welcome");
    setLine(""); setGreetingArmed(true); // titulky i mikrofon až po pozdravu
    clearTimeout(greetTimer.current);
    greetTimer.current = setTimeout(() => { setLine(w); say(w); }, GREET_DELAY);
  }
  ...
  function idle() { clearTimeout(greetTimer.current); setGreetingArmed(false); resetAll(); setScreen("idle"); setLine(""); }
  ...
  // Hero layout úvodu jen když je aktivní D-ID avatar (jinak klasický rozcestník).
  const heroActive = HERO_HOME && didActive && screen === "home";
  // Epic = Daniel vlevo + velké okno vpravo pro VŠECHNY obrazovky (ne jen úvod).
  const epicActive = HERO_EPIC && didActive && screen !== "idle";

  // Tlačítka rozcestníku (sdílená klasickým i hero layoutem, ať se nerozjedou).
  const homeChoices = (
    <>
      {recSupported && (
      ...
      {/* hlas = bonus: během poslechu hezká zpětná vazba „slyším…", chyby tiše ignorujeme */}
      {listening && recInterim && <div className="voice-heard">„{recInterim}"</div>}
      {!listening && heard && <div className="voice-heard">„{heard}"</div>}
  ...
  // ─────────────────────────────────────────────────────────
  return (
    <div className={`kiosk${epicActive ? " epic-mode" : ""}`}>
      {screen === "idle" ? (
        <div className="idle" onClick={wake}>
          {USE_DID
            ? <img className="did-idle" src={DID_POSTER} alt="" style={{ width: 320, height: 320 }} />
            : <Avatar speaking={false} line="" variant={AVATAR} size={180} />}
          {/* přednačti uvítací klip, ať po klepnutí naskočí hned (a Daniel mluví s pusou) */}
          {USE_DID && clipFor(t("welcome")) && (
            <video src={clipFor(t("welcome"))} preload="auto" muted playsInline style={{ display: "none" }} />
          )}
        ...
        <>
          {/* EPIC: Daniel trvale vlevo (přes celou výšku) + částicové pozadí; obsah jde do .shell vpravo. */}
          {epicActive && <Particles />}
          {epicActive && (
            <div className="epic-figure">
              <AvatarStage ref={stageRef} lang={lang} className="full" width={620} height={900} poster={DID_POSTER} onSpeaking={onAvatarSpeaking} />
              {screen === "home" && avatarSpeaking && line && <div className="epic-caption">{line}</div>}
            </div>
          )}
          <div className="shell">
          ...
          {heroActive && HERO_BOLD ? (
            /* BOLD úvod: velký portrét Daniela (bez kruhu) + skleněný panel s tlačítky (?home=bold). */
            <div className="hero bold">
              <div className="hero-left">
                <div className="hero-aura" />
                <AvatarStage ref={stageRef} lang={lang} className="portrait" width={440} height={620} poster={DID_POSTER} />
              </div>
              <div className="hero-panel">
                <div className="hero-eyebrow">🛎️ {property.name}</div>
                <div className="hero-title">{line || t("welcome")}</div>
                <div className="hero-actions">{homeChoices}</div>
              </div>
            </div>
          ) : heroActive ? (
            /* HERO úvod: velký Daniel (kruh) vlevo, tlačítka vpravo (?home=hero). */
            <div className="hero">
              <div className="hero-left">
                <div className="hero-glow" />
                <AvatarStage ref={stageRef} lang={lang} size={460} poster={DID_POSTER} />
                <div className="hero-name">Daniel</div>
                <div className="hero-greet">{line}</div>
              </div>
              <div className="hero-right">{homeChoices}</div>
            </div>
          ) : (
          <>
          ...
          {!epicActive && (didLive
            ? <AvatarStage ref={stageRef} lang={lang} size={300} poster={DID_POSTER} onSpeaking={onAvatarSpeaking} />
            : USE_DID
              ? <img className="did-idle" src={DID_POSTER} alt="" style={{ width: 300, height: 300 }} />
              : <Avatar speaking={speaking} line={screen === "home" ? line : ""} variant={AVATAR} size={120} />)}

          <div className="content">
            {screen === "home" && (
              <div className="choices">{homeChoices}</div>
            )}
